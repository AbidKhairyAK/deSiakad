<template>
	<div>
		<page-header title="Mata Pelajaran" text="Berisi daftar data Mata Pelajaran di sekolah"></page-header>
		...
		<div class="row">
			...
			<div class="col-md-9">
				<box title="Tabel Mata Pelajaran" 
	...
	export default {
		...
		methods: {
			getData() {
				...
				this.$axios.get(`${this.$url}/mapel?${query}`).then((response) => {
			...
			sendData(item) {
				...
				? `${this.$url}/mapel/${item.id}/update`
				: `${this.$url}/mapel/create`;
			...
			destroyData(id) {
				if (confirm('apakah anda yakin?')) {
					this.$axios.delete(`${this.$url}/mapel/${id}/destroy`).then(() => {
						this.getData();
					});
				}
			},

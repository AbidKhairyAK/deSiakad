<template>
	<div>
		<page-header title="Daftar Siswa" text="Berisi daftar data seluruh siswa"></page-header>
...
<script>
	...
	export default {
		...
		data: () => ({
			url_query: {
				page: 1, angkatan: '', agama: '', gender: '', search: ''
			},
			item: {},
			api: {},
			mode: null,
		...
		methods: {
			getData() {
				...
				this.$axios.get(`${this.$url}/siswa?${query}`).then((response) => {

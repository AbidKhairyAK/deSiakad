<template>
	<div>
		<page-header title="Tahun Akademik" text="Berisi daftar data Tahun Akademik"></page-header>
		...
		<div class="row">
			...
			<div class="col-md-9">
				<box title="Tabel Tahun Akademik" 
	...
	export default {
		...
		methods: {
			getData() {
				...
				this.$axios.get(`${this.$url}/tahun-akademik?${query}`).then((response) => {
			...
			sendData(item) {
				...
				? `${this.$url}/tahun-akademik/${item.id}/update`
				: `${this.$url}/tahun-akademik/create`;
			...
			destroyData(id) {
				if (confirm('apakah anda yakin?')) {
					this.$axios.delete(`${this.$url}/tahun-akademik/${id}/destroy`).then(() => {
						this.getData();
					});
				}
			},

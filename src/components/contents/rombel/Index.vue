<template>
	<div>
		<page-header title="Rombongan Belajar" text="Berisi daftar data rombongan belajar siswa"></page-header>
		...
		<div class="row">
			...
			<div class="col-md-9">
				<box title="Tabel Rombongan Belajar" 
	...
	export default {
		...
		methods: {
			getData() {
				...
				this.$axios.get(`${this.$url}/rombel?${query}`).then((response) => {
			...
			sendData(item) {
				...
				? `${this.$url}/rombel/${item.id}/update`
				: `${this.$url}/rombel/create`;
			...
			destroyData(id) {
				if (confirm('apakah anda yakin?')) {
					this.$axios.delete(`${this.$url}/rombel/${id}/destroy`).then(() => {
						this.getData();
					});
				}
			},

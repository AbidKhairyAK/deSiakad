<template>
	<div>
		<page-header title="Jenis Pembayaran" text="Berisi daftar data jenis pembayaran di sekolah"></page-header>
		...
		<div class="row">
			...
			<div class="col-md-9">
				<box title="Tabel Jenis Pembayaran" 
	...
	export default {
		...
		methods: {
			getData() {
				...
				this.$axios.get(`${this.$url}/jenis-pembayaran?${query}`).then((response) => {
			...
			sendData(item) {
				...
				? `${this.$url}/jenis-pembayaran/${item.id}/update`
				: `${this.$url}/jenis-pembayaran/create`;
			...
			destroyData(id) {
				if (confirm('apakah anda yakin?')) {
					this.$axios.delete(`${this.$url}/jenis-pembayaran/${id}/destroy`).then(() => {
						this.getData();
					});
				}
			},

<template>
	<div>
		<page-header title="Daftar Guru" text="Berisi daftar data seluruh guru"></page-header>
...
<script>
	...
	export default {
		...
		data: () => ({
			url_query: {
				page: 1, status: '', agama: '', gender: '', search: ''
			},
			item: {},
			api: {},
			mode: null,
		...
		methods: {
			getData() {
				...
				this.$axios.get(`${this.$url}/guru?${query}`).then((response) => {
			...
			sendData(item) {
				...
				? `${this.$url}/guru/${item.id}/update`
				: `${this.$url}/guru/create`;

				this.$axios.post(url, item, this.$url_config).then(({data}) => {
					if (this.item.foto_data) {
						this.sendFoto(item, data.id);
					}
					this.getData();
					this.closeModal();
				});
			...
			sendFoto(item, id) {
				...
				formData.set('foto', item.foto_data);

				this.$axios.post(`
					${this.$url}/guru/${id}/add-foto`, 
					formData, 
			...
			destroyData(id) {
				if (confirm('apakah anda yakin?')) {
					this.$axios.delete(`${this.$url}/guru/${id}/destroy`).then(() => {
						this.getData();
					});
				}
			},

<template>
	<div>
		<page-header title="Jadwal Pelajaran" text="Berisi jadwal pelajaran siswa"></page-header>
		...
		<div class="row">
			...
			<div class="col-md-9">
				<box title="Tabel Jadwal Pelajaran" 
				...
				>
					<template #right-header>
						<button class="btn btn-sm btn-primary rounded" @click="toEditor()">
							<i class="fa fa-edit"></i> Atur Jadwal Pelajaran
						</button>
					</template>
...
<script>
	...
	export default {
		...
		data: () => ({
			url_query: {
				page: 1, rombel: '', mapel: '', guru: '', ruangan: '', jam: '', hari: '', search: ''
			},
			item: {},
			api: {},
			mode: null,
		...
		methods: {
			getData() {
				...
				this.$axios.get(`${this.$url}/jadwal?${query}`).then((response) => {
			...
			toEditor() {
				this.$router.push('jadwal-pelajaran-editor');
			}
		},
